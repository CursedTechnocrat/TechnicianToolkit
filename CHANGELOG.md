...
### Fixed
- **`S.H.A.D.E.` never staged the shared module, so remote tools failed on isolated endpoints.** `Invoke-RemoteTool` and `Invoke-RemoteSigil` copied only the tool's own `.ps1` into `C:\Temp\ShadeToolkit`, but every tool begins with the module bootstrap, which requires `TechnicianToolkit.psm1` beside the script. With the module absent the remote copy fell through to its GitHub download path — and on the isolated or firewalled machines SHADE is most often pointed at, that download fails and the bootstrap calls `exit 1`, so the run produced no report and no clear reason. Both functions now copy `TechnicianToolkit.psm1` alongside the tool (warning if it is missing locally), so the remote bootstrap resolves the module from disk and never needs a route to GitHub.
- **`S.H.A.D.E.` invoked remote tools interactively, stalling the run.** Both remote entry points ran the staged tool as a bare `& $scriptPath`, with no arguments. A WinRM runspace has no console, so the first `Read-Host` in AUSPEX, WARD, or RESTORATION had nothing to read and the session hung rather than returning. `Invoke-RemoteTool` now takes a `[hashtable]$ScriptArgs` (defaulting to `@{ Unattended = $true }`) and splats it at the call site; a hashtable splat is required here because an array splat binds positionally and would pass `-Unattended` as a *value* rather than setting the switch. `Invoke-RemoteSigil` now calls `sigil.ps1 -Unattended -Categories A`, which is what its own comment ("piping `A` to select all categories") had always claimed it did but never implemented. The dead `[scriptblock]$RemoteArgs` parameter it replaced was never referenced. SHADE's RESTORATION warning was corrected to match the actual behaviour: the tool runs without `-AutoReboot`, so a required reboot is reported, not performed.
- **`T.H.R.E.S.H.O.L.D.` reported EFI and recovery partitions as near-full volumes.** `Get-VolumeInfo` filtered with `$_.DriveLetter -ne $null`, but `Get-Volume` returns `[char]0` — not `$null` — for a volume with no drive letter, and `[char]0 -ne $null` is `$true`. Every letterless volume therefore passed the filter, so the EFI system partition and the recovery partitions were listed as ordinary volumes with a blank drive letter and, because they are small and largely full by design, were classified `Warning` or `Critical` and raised false low-space alerts. The filter now tests `$_.DriveLetter` for truthiness, which correctly rejects `[char]0` while still admitting a real letter.
- **`C.O.V.E.N.A.N.T.` join-state flags held matched text instead of booleans.** The Entra ID / domain / workplace join checks were written as `($dsregOutput | Where-Object { … }) -ne $null`. Applied to a pipeline result, `-ne` *filters the collection* rather than testing it, so each variable held the matched `dsregcmd` output line (or an array of them) instead of `$true`/`$false`. The `if` branches happened to behave because a non-empty string is truthy, but the values were never the booleans the code read as. All four sites now use `@(…).Count -gt 0`.

### Changed
- **`P.O.R.T.A.L.` now surfaces the insecure-VPN count, and `G.O.L.E.M.` the unknown-compliance count.** Both figures were computed and then dropped before the summary row was built. PORTAL's `$insecureCount` — the number of VPNs whose auth or encryption tier resolves to `Insecure`, which is the tool's headline finding — now has its own card, red when non-zero. GOLEM's `$unknownCount` (devices whose Intune compliance state is `unknown`) now has a card too; without it the compliance cards never reconciled with the managed-device total.
- **Automatic-variable shadowing removed in `B.E.A.C.O.N.` and `G.R.I.M.O.I.R.E.`** BEACON's WLAN XML parser assigned to `$profile`, which is PowerShell's automatic profile-path variable; it is now `$wlanProfile` (the unrelated plural `$profiles` is untouched). GRIMOIRE's `Select-FromMatches` declared a parameter named `$Matches`, shadowing the automatic hashtable that any `-match` operator overwrites — a latent trap for anyone adding a regex to that function. The parameter is now `-Found`, with both call sites updated.

### Removed
- **Dead assignments cleared from five tools.** `auspex.ps1`'s `$batteryHtml` builder (the System Health block already renders battery inline, so it was a leftover from an earlier refactor), `threshold.ps1`'s unread `$diskNum`, `lantern.ps1`'s `$portScanned` (whose `-ne $null` test on an array was wrong anyway), and `reliquary.ps1`'s `$totalLicensed` / `$inactiveCount` / `$noMfaCount` — three duplicates of values the summary cards already read from their source collections, one of which (`$totalLicensed`) actually held the *un*licensed count. No output changes.
- **UTF-8 BOM added to `.github/PSScriptAnalyzerSettings.psd1`.** The only file in the repository still missing one after the 3.6.1 sweep, and the last `PSUseBOMForUnicodeEncodedFile` warning.

### Notes
- PSScriptAnalyzer (repo settings, `PSAvoidUsingWriteHost` excluded): **0 errors**, warnings down from 218 to 198. `PSPossibleIncorrectComparisonWithNull` (10), `PSAvoidAssignmentToAutomaticVariable` (2), and `PSUseBOMForUnicodeEncodedFile` (1) are now fully cleared; `PSUseDeclaredVarsMoreThanAssignments` drops from 22 to 14. The remaining `PSReviewUnusedParameter` and `PSUseUsingScopeModifierInNewRunspaces` hits were each checked and are false positives — the parameters are used inside nested scopes, and the `Invoke-Command` script blocks declare their own `param()` and receive values via `-ArgumentList`.
